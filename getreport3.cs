...
class Apitester
{
    static async Task Main(string[] args)
    {
        ...
        using (HttpClient client = new HttpClient())
        {
            ...
            HttpResponseMessage response = await client.GetAsync($"/Reports/29/RunReport?parameters=ChargeTypeOrder,(69,70,71,72,73,74,75,76,77,78,79,80);StartDate,{startDate};EndDate,{endDate}&GetOptions=ReturnExcelStream");
            try
            {
            ...
            string reportFile = Path.Combine(reportPath, $"ChargeBreakdown{currDate}frag3.xlsx");
