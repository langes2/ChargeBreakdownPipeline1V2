...
class Apitester
{
    static async Task Main(string[] args)
    {
        ...
        using (HttpClient client = new HttpClient())
        {
            ...
            HttpResponseMessage response = await client.GetAsync($"/Reports/29/RunReport?parameters=ChargeTypeOrder,(57,58,59,60,61,62,63,64,65,66,67,68);StartDate,{startDate};EndDate,{endDate}&GetOptions=ReturnExcelStream");
            try
            {
            ...
            string reportFile = Path.Combine(reportPath, $"ChargeBreakdown{currDate}frag2.xlsx");
